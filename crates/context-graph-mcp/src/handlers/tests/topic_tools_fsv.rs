...
/// FSV Test: Verify cognitive pulse is included in all topic tool responses
///
/// Source of Truth: _cognitive_pulse field in response
/// Validates: All topic tools include live UTL metrics
#[tokio::test]
async fn test_fsv_cognitive_pulse_included() {
    println!("\n=== FSV Test: Verify _cognitive_pulse in topic tool responses ===");

    let (handlers, _, _tempdir) = create_test_handlers_with_rocksdb_store_access().await;

    // Test all 4 topic tools
    let tools = [
        ("get_topic_portfolio", json!({})),
        ("get_topic_stability", json!({})),
        ("get_divergence_alerts", json!({})),
    ];
    ...
    for (tool_name, args) in tools {
        ...
        // Skip if error (detect_topics will error due to insufficient memories)
        if response.error.is_some() {
            println!("  SKIP (returned JSON-RPC error)");
            continue;
        }

        let result = response.result.expect("Must have result");

        // Verify _cognitive_pulse is present
        let pulse = result.get("_cognitive_pulse");
        assert!(
            pulse.is_some(),
            "{} must include _cognitive_pulse",
            tool_name
        );
    ...
    println!("\n[FSV PASS] All topic tools include _cognitive_pulse");
}
