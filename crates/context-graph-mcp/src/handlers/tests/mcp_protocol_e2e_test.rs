...
async fn test_e2e_mcp_handshake_with_gpu() {
    ...
    let (handlers, _store, _tempdir) = create_test_handlers_with_real_embeddings_store_access().await;
...
async fn test_e2e_tools_list_all_14_tools() {
    ...
    for tool in tools {
        ...
        println!("  - {} (desc: {}, schema: {})", name, has_description, has_schema);
    }
...
async fn test_e2e_core_tools_workflow() {
    ...
    let inject_request = make_request("tools/call", Some(JsonRpcId::Number(1)), Some(inject_params));
    ...
    let inject_result = inject_response.result.expect("inject_context must return result");
    ...
    assert!(store_response.error.is_none(), "store_memory must not error");
    let store_result = store_response.result.expect("store_memory must return result");
    ...
    let status_request = make_request("tools/call", Some(JsonRpcId::Number(3)), Some(status_params));
    let status_response = handlers.dispatch(status_request).await;

    assert!(status_response.error.is_none(), "get_memetic_status must not error");
    let status_result = status_response.result.expect("get_memetic_status must return result");
    ...
    let search_request = make_request("tools/call", Some(JsonRpcId::Number(4)), Some(search_params));
    let search_response = handlers.dispatch(search_request).await;

    assert!(search_response.error.is_none(), "search_graph must not error");
    let search_result = search_response.result.expect("search_graph must return result");
    ...
    for (i, result) in results.iter().enumerate() {
        let similarity = result.get("similarity").and_then(|v| v.as_f64()).unwrap_or(0.0);
        let dominant = result
...
async fn test_e2e_topic_tools_workflow() {
    ...
    let portfolio_request = make_request("tools/call", Some(JsonRpcId::Number(10)), Some(portfolio_params));
    ...
    let portfolio_result = portfolio_response.result.expect("get_topic_portfolio must return result");
    let is_error = portfolio_result
        .get("isError")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    assert!(!is_error, "get_topic_portfolio should not return tool error");
    ...
    let stability_request = make_request("tools/call", Some(JsonRpcId::Number(11)), Some(stability_params));
    ...
    let stability_result = stability_response.result.expect("get_topic_stability must return result");
    let is_error = stability_result
        .get("isError")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    assert!(!is_error, "get_topic_stability should not return tool error");
    ...
    let detect_request = make_request("tools/call", Some(JsonRpcId::Number(12)), Some(detect_params));
    ...
    let divergence_request = make_request("tools/call", Some(JsonRpcId::Number(13)), Some(divergence_params));
...
async fn test_e2e_curation_tools_workflow() {
    ...
    let boost_request = make_request("tools/call", Some(JsonRpcId::Number(10)), Some(boost_params));
    ...
    let boost_result = boost_response.result.expect("boost_importance must return result");
    ...
    let forget_request = make_request("tools/call", Some(JsonRpcId::Number(11)), Some(forget_params));
    ...
    let forget_result = forget_response.result.expect("forget_concept must return result");
    let is_error = forget_result
        .get("isError")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    assert!(!is_error, "forget_concept should not return tool error");
    println!("forget_concept: success (soft-deleted {})", fingerprint_ids[2]);
    ...
    let merge_request = make_request("tools/call", Some(JsonRpcId::Number(12)), Some(merge_params));
    ...
    let merge_result = merge_response.result.expect("merge_concepts must return result");
    ...
    let consolidation_request = make_request("tools/call", Some(JsonRpcId::Number(13)), Some(consolidation_params));
...
async fn test_e2e_dream_tools_workflow() {
    ...
    let dream_request = make_request("tools/call", Some(JsonRpcId::Number(10)), Some(dream_params));
    ...
    let dream_result = dream_response.result.expect("trigger_dream must return result");
    ...
    let status_request = make_request("tools/call", Some(JsonRpcId::Number(11)), Some(status_params));
    let status_response = handlers.dispatch(status_request).await;
    ...
    let status_result = status_response.result.expect("get_dream_status must return result");
...
async fn test_e2e_all_14_tools_callable() {
    ...
    let store_result = call_tool(&handlers, "store_memory", json!({
    ...
    // Core tools (5)
    println!("\n--- Core Tools ---");
    if call_tool(&handlers, "inject_context", json!({
    ...
    if call_tool(&handlers, "search_graph", json!({
        "query": "test",
        "topK": 5
    })).await {
        successful_tools.insert("search_graph");
    }
    if call_tool(&handlers, "trigger_consolidation", json!({
        "strategy": "similarity"
    })).await {
    ...
    // Topic tools (4)
    println!("\n--- Topic Tools ---");
    if call_tool(&handlers, "get_topic_portfolio", json!({
        "format": "brief"
    })).await {
        successful_tools.insert("get_topic_portfolio");
    }
    if call_tool(&handlers, "get_topic_stability", json!({
        "hours": 1
    })).await {
        successful_tools.insert("get_topic_stability");
    }
    if call_tool(&handlers, "detect_topics", json!({
        "force": false
    })).await {
        successful_tools.insert("detect_topics");
    }
    if call_tool(&handlers, "get_divergence_alerts", json!({
        "lookback_hours": 1
    })).await {
    ...
    // Curation tools (3)
    println!("\n--- Curation Tools ---");
    if call_tool(&handlers, "boost_importance", json!({
        "node_id": &fingerprint_id,
        "delta": 0.1
    })).await {
        successful_tools.insert("boost_importance");
    }
    if call_tool(&handlers, "forget_concept", json!({
        "node_id": &fingerprint_id,
        "soft_delete": true
    })).await {
    ...
    // Dream tools (2)
    println!("\n--- Dream Tools ---");
    if call_tool(&handlers, "trigger_dream", json!({
        "blocking": true,
        "dry_run": true
    })).await {
