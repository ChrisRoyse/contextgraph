...
**Current variants** (as of commit 1e19466):
```rust
pub enum EmbeddingError {
    ModelLoadError(String),           // ❌ Wrong signature - needs ModelId + source
    GenerationError(String),          // ❌ Not in spec - REMOVE
    InvalidInput(String),             // ❌ Wrong - should be EmptyInput
    DimensionMismatch { expected, actual }, // ✅ Matches InvalidDimension
    InvalidVector(String),            // ❌ Wrong - should be InvalidValue { index, value }
    ExpertWeightsInvalid(String),     // ❌ Not in spec - MOVE to FuseMoE errors
    InvalidExpertIndex { index, max }, // ❌ Not in spec - MOVE to FuseMoE errors
    SerializationError(String),       // ✅ Keep but needs { message } format
    CompressionError(String),         // ❌ Not in spec - REMOVE
    InvalidAuxData(String),           // ❌ Not in spec - REMOVE
    IoError(#[from] std::io::Error),  // ✅ Correct
}
```
...
- Both are exported via `types/mod.rs`
...
*Audited Against: commit 1e19466*
