...
- [ ] `entity.rs` created at `crates/context-graph-embeddings/src/models/pretrained/entity.rs`
- [ ] Constants defined: `ENTITY_DIMENSION`, `ENTITY_MAX_TOKENS`, `ENTITY_LATENCY_BUDGET_MS`, `ENTITY_MODEL_NAME`
- [ ] `EntityModel` struct with `model_state`, `model_path`, `config`, `loaded`, `memory_size`
- [ ] `ModelState` enum with `Unloaded`, `Stub` variants
- [ ] `new()` constructor with zero batch size validation
- [ ] `encode_entity()` static method - uppercases type
- [ ] `encode_relation()` static method - replaces underscores
- [ ] `transe_score()` static method - returns negative L2 distance
- [ ] `predict_tail()` static method - returns h + r
- [ ] `predict_relation()` static method - returns t - h
- [ ] `load()` async method - sets `loaded` to true
- [ ] `unload()` async method - checks initialized first, sets `loaded` to false
- [ ] `embed_batch()` async method
- [ ] `EmbeddingModel` trait: `model_id()`, `supported_input_types()`, `is_initialized()`, `embed()`
- [ ] `Send + Sync` implemented
- [ ] `generate_deterministic_embedding()` helper using xxhash + L2 normalization
- [ ] `extract_content()` helper for ModelInput text extraction
- [ ] Module exported in `pretrained/mod.rs`
- [ ] 40+ tests written and passing
- [ ] 5 edge case tests with before/after logging
- [ ] Source of truth verification test
- [ ] Evidence of success test
- [ ] `cargo check` passes
- [ ] `cargo test` passes
